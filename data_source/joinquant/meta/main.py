...
class main(__DB_CLASS__, META_DATABASE):
    ...
    def __init__(self, **kwargs):
        parameters = filter_class_attrs(DATABASE) | kwargs | filter_class_attrs(FILTER)
    ...
    def __columns_rename__(self, df):
        if isinstance(self.columns_information, dict):
    ...
    def __get_data_from_jq_remote__(self, **kwargs):
        df = eval(self.jq_command.format(**kwargs))
        return df            

    def __data_standard__(self, df, **kwargs):
    ...
    def __find_max_of_exist_table__(self, columns: str, **kwargs) -> Union[int, float]:
        id_key = None
        if self.table_exist():
            id_key = self.__read__(columns='MAX({})'.format(columns), show_time=False, **kwargs).iloc[0, 0]
    ...
    def create_table(self, **kwargs):
        ...
        if self.source == 'MySQL':
            keys = self.ann_dt if self.trade_dt not in self.columns.keys() else self.trade_dt
            partition = None if keys != self.trade_dt else {self.trade_dt:self.partition}
            parameters = self.__parameters__() | {'keys':keys, 'partition' :partition}  | {'columns':self.columns, 'log':True} | kwargs
        else:
