...
from data_source.local import LOGIN_INFO
import jqdatasdk as jq
jq.auth(**LOGIN_INFO)

class DATABASE(BASE_DATABASE_CONFIG):
    schema: str = 'jq_data'
    partition: pd.DatetimeIndex = pd.date_range('2005-01-01', '2030-12-31', freq='YE')
...
class ANN_DT_TABLES:
    asharebalancesheet: Dict[str, Union[str, Any]] = {
        'table': 'asharebalancesheet',
        'columns_information': jq.finance.STK_BALANCE_SHEET, 
        'jq_command': 'jq.finance.run_query(jq.query(jq.finance.STK_BALANCE_SHEET).order_by(jq.finance.STK_BALANCE_SHEET.id).filter((jq.finance.STK_BALANCE_SHEET.source_id == 321003) & (jq.finance.STK_BALANCE_SHEET.report_type == 0) & (jq.finance.STK_BALANCE_SHEET.id > {id_key})).limit(5000))',
    }
    
    asharecashflow: Dict[str, Union[str, Any]] = {
        'table': 'asharecashflow',
        'columns_information': jq.finance.STK_CASHFLOW_STATEMENT, 
        'jq_command': 'jq.finance.run_query(jq.query(jq.finance.STK_CASHFLOW_STATEMENT).order_by(jq.finance.STK_CASHFLOW_STATEMENT.id).filter((jq.finance.STK_CASHFLOW_STATEMENT.source_id == 321003) & (jq.finance.STK_CASHFLOW_STATEMENT.report_type == 0) & (jq.finance.STK_CASHFLOW_STATEMENT.id > {id_key})).limit(5000))',
    }
    
    ashareincome: Dict[str, Union[str, Any]] = {
        'table': 'ashareincome',
        'columns_information': jq.finance.STK_INCOME_STATEMENT, 
        'jq_command': 'jq.finance.run_query(jq.query(jq.finance.STK_INCOME_STATEMENT).order_by(jq.finance.STK_INCOME_STATEMENT.id).filter((jq.finance.STK_INCOME_STATEMENT.source_id == 321003) & (jq.finance.STK_INCOME_STATEMENT.report_type == 0) & (jq.finance.STK_INCOME_STATEMENT.id > {id_key})).limit(5000))',
    }
    
    ashareperformanceletter: Dict[str, Union[str, Any]] = {
        'table': 'ashareperformance_lt',
        'columns_information': jq.finance.STK_PERFORMANCE_LETTERS, 
        'jq_command': 'jq.finance.run_query(jq.query(jq.finance.STK_PERFORMANCE_LETTERS).order_by(jq.finance.STK_PERFORMANCE_LETTERS.id).filter((jq.finance.STK_PERFORMANCE_LETTERS.report_type == 0) & (jq.finance.STK_PERFORMANCE_LETTERS.id > {id_key})).limit(5000))',
    }
    
    ashareforcast: Dict[str, Union[str, Any]] = {
        'table': 'ashareforcast',
        'columns_information': jq.finance.STK_FIN_FORCAST, 
        'jq_command': 'jq.finance.run_query(jq.query(jq.finance.STK_FIN_FORCAST).order_by(jq.finance.STK_FIN_FORCAST.id).filter(jq.finance.STK_FIN_FORCAST.id > {id_key}))',
    }
    
    asharedividend: Dict[str, Union[str, Any]] = {      
        'table': 'asharedividend', 
        'columns_information': jq.finance.STK_XR_XD, 
        'jq_command': 'jq.finance.run_query(jq.query(jq.finance.STK_XR_XD).order_by(jq.finance.STK_XR_XD.id).filter((jq.finance.STK_XR_XD.id > {id_key}) & (jq.finance.STK_XR_XD.report_date >= "2004-01-01")))'
    }
    
    asharestatus: Dict[str, Union[str, Any]] = {
        'table': 'asharestatus',
        'columns_information': jq.finance.STK_STATUS_CHANGE,
        'jq_command': 'jq.finance.run_query(jq.query(jq.finance.STK_STATUS_CHANGE).order_by(jq.finance.STK_STATUS_CHANGE.id).filter((jq.finance.STK_STATUS_CHANGE.id > {id_key})).limit(5000))'
    }    
    
class TRADE_DT_TABLES:
    ashareeodprices: Dict[str, Union[str, Dict[str, List[Union[str, int]]], Any]] = {   
        'table':'ashareeodprices',         
        'columns_information':{'time': {'TRADE_DT': ['datetime', '交易日']},
                             ...
                             'pre_close_adj': {'S_DQ_PRECLOSE_ADJ': ['double(18, 8)', '后复权昨收盘价(元)']},
    ...
    ashareeodderivativeindicator: Dict[str, Union[str, Dict[str, List[Union[str, int]]], Any]] = {
        'table': 'ashareeodderivativeindicator',
        'columns_information': {'day': {'TRADE_DT': ['datetime', '交易日']},
         'code': {'S_INFO_WINDCODE': ['VARCHAR(16)', '股票代码']},
         ...
         'circulating_market_cap': {'S_DQ_MV': ['double(20, 4)', '流通市值']},
         'turnover_ratio': {'S_DQ_FREETURNOVER': ['double(20, 4)',
           '换手率\n[指定交易日成交量(手)100/截至该日股票的自由流通股本(股)]*100%']},
         'pe_ratio': {'S_DQ_PE_TTM': ['double(20, 4)',
           '市盈率(PE, TTM)\n市盈率（PE，TTM）=（股票在指定交易日期的收盘价 * 当日人民币外汇挂牌价* 截止当日公司总股本）/归属于母公司股东的净利润TTM']},
         'pe_ratio_lyr': {'S_DQ_PE_LYR': ['double(20, 4)',
           '市盈率(PE, LYR)\n市盈率（PE）=（股票在指定交易日期的收盘价 * 当日人民币外汇牌价 * 截至当日公司总股本）/归属母公司股东的净利润']},
         'pb_ratio': {'S_DQ_PB': ['double(20,4)',
           '市净率(PB)\n市净率=（股票在指定交易日期的收盘价 * 当日人民币外汇牌价 * 截至当日公司总股本）/归属母公司股东的权益']},
         'ps_ratio': {'S_DQ_PS_TTM': ['double(20, 4)',
           '市销率(PS, TTM)\n市销率TTM=（股票在指定交易日期的收盘价 * 当日人民币外汇牌价 * 截至当日公司总股本）/营业总收入TTM']},
         'pcf_ratio': {'S_DQ_PCF_TTM': ['double(20, 4)',
           '市现率(PCF, 现金净流量TTM)\n市现率=（股票在指定交易日期的收盘价 * 当日人民币外汇牌价 * 截至当日公司总股本）/现金及现金等价物净增加额TTM']},
         'pcf_ratio2': {'S_DQ_POCF_TTM': ['double(20, 4)',
           '市现率(PCF, 经营现金净流量TTM)\n市现率=（股票在指定交易日期的收盘价 * 截至当日公司总股本）/经营活动现金及经营活动现金等价物净增加额TTM']},
         'dividend_ratio': {'S_DQ_DIVRATIO_TTM': ['double(20, 4)',
           '(近12个月派现合计/总市值)/100']},
         'free_market_cap': {'S_FREE_MV': ['double(20, 4)',
           '自由流通市值(亿元), A股收盘价*自由流通股本, 自由流通股本 = 流通股本-其他扣除数(如高管限售25%)']}},
        'jq_command': 'jq.get_fundamentals(jq.query(jq.valuation), date="{date}")'
    }
    
    
    ashareindicator: Dict[str, Union[str, Any]] = { 
    ...
    aindexeodprices: Dict[str, Union[str, List[str], Dict[str, List[Union[str, int]]], Any]] = {
        'table': 'aindexeodprices',
        'security':  ['000016.XSHG', '000852.XSHG', '000905.XSHG', '399300.XSHE', '399303.XSHE', '000985.XSHG'],
        'fields': ['avg', 'close', 'high', 'low', 'money', 'open', 'pre_close', 'volume'],
        'columns_information': {'time': {'TRADE_DT': ['datetime', '交易日']},
                     'code': {'S_INFO_WINDCODE': ['VARCHAR(16)',
    ...
    aindexweights: Dict[str, Union[str, List[str], Dict[str, List[Union[str, int]]], Any]] = {
            'table':'aindexweights',    
            'security': ['000016.XSHG', '000852.XSHG', '000905.XSHG', '399300.XSHE', '399303.XSHE', '000985.XSHG'],
            'fields': ['date', 'weight'],
            'columns_information':{'level_1': {'S_INFO_WINDCODE': ['VARCHAR(16)', '股票代码: 000016(上证50), 000852(中证1000指数), 000905(中证500), 399300(沪深300), 399303(国证2000), 000985(中证全指)']},
    ...
    ashareindustrys: Dict[str, Union[str, Dict[str, List[Union[str, Tuple[str, str]]]], Any]] = { 
        'table': 'ashareindustrys',
        'columns_information': {('', ''): {'TRADE_DT': ['datetime', '交易日', ]},
    ...
    asharelisting: Dict[str, Union[str, Dict[str, List[Union[str, int]]], Any]] = { 
        'table': 'asharelisting',
        'columns_information':{'index': {'S_INFO_WINDCODE': ['VARCHAR(16)', '股票代码', ]},
    ...
    ashareconcept: Dict[str, Union[str, Dict[str, List[Union[str, int]]], Any]] = {       
        'table': 'ashareconcept',
        'columns_information':{'level_1': {'TRADE_DT': ['datetime', '交易日']},
